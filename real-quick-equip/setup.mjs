export function setup(ctx) {
  ...
  const quickEquip = () => {
    ...
    items
      .filter((x) => game.stats.itemFindCount(x) > 0)
      .forEach((item) => {
    ...
    if (passive.length > 0 && game.combat.player.isEquipmentSlotUnlocked(passiveSlot)) {
      ...
      if (passive.length > 0) {
        if (passive.length == 1) {
          ...
          notifyPlayer(
            skill,
            `${templateLangString("EQUIP_SLOT_11")} conflict: ${passive.map((x) => itemMap[x].name).join(" / ")}`,
            "danger"
          );
          return;
        }
    ...
    if (equipment["melvorD:Enhancement1"]) {
      const length = equipment["melvorD:Enhancement1"].length;

      if (length == 2) {
        equipment["melvorD:Enhancement2"] = [equipment["melvorD:Enhancement1"].pop()];
      } else if (length == 3) {
        equipment["melvorD:Enhancement2"] = [equipment["melvorD:Enhancement1"].pop()];
        equipment["melvorD:Enhancement3"] = [equipment["melvorD:Enhancement1"].pop()];
      }
    }
  ...
  ctx.patch(Minibar, "initialize").after(() => {
    const quickEquipReal = game.minibar.createMinibarItem(
      "minibar-quick-equip-real",
      `${CDNDIR()}assets/media/bank/fez_amulet.png`,
      '<div class="text-center"><small>Click to Equip All</small></div>',
      {
        onClick: () => quickEquip(),
      }
    );
  ...
  const equipAmmo = () => {
    ...
    const items = game.bank.filterItems((bankItem) => {
      ...
      return (
        bankItem.item instanceof EquipmentItem &&
        bankItem.item.ammoType == ammoType &&
        game.checkRequirements(bankItem.item.equipRequirements)
      );
    });
  ...
  const quickEquipSynergyString = game.combat.player.quickEquipSynergy
    .toString()
    .replace(/^[^{]*{/, "")
    .replace(/}$/, "")
